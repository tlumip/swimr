...
#' Compare socioeconomic variables on a ggmap
#'
#'
#' @importFrom sp bbox
#' @importFrom ggmap get_map ggmap
#' @export
change_ggmap <- function(db1, db2, scope = NULL, year = 2010,
                         metric = c("diff", "pct"),
                         variable = c("Population", "Employment", "HH"),
                         scen_names = c("Reference", "Current"),
                         ggmap = FALSE){
  ...
  if(metric == "diff"){
    se <- mutate(se, cuts = cut_abserror(diff))
    metlabel <- "Absolute"
  } else {
    se <- mutate(se, cuts = cut_diverror(pct))
    metlabel <- "%"
  }
  ...
  p + geom_polygon(
    ...
    alpha = alphamark,
    size = 0.01,
    aes_string(x = "long", y = "lat", fill = "cuts",
               group = "group")
  ) +
    scale_fill_brewer(paste0(
      metlabel, " change per sqmi\n",
      scen_names[1], " - ", scen_names[2]),
      palette = "PRGn")
