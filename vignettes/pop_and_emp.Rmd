...
```{r plot_sevar}
plot_sevar(db, color_var = facet, color_levels = counties)
```
...
```{r plot_employment}
plot_employment(db, facet_var = facet, facet_levels = counties)
```
...
```{r floorspace}
plot_floorspace(db, facet_var = facet,
                facet_levels = counties)
```
